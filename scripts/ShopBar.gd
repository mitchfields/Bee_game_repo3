...
func _spawn_shop_items() -> void:
	...
	if item_scenes.is_empty():
		push_error("ShopBar: no scenes assigned")
		return
...
func _reflow_items() -> void:
	for i in range(items.size()):
		var it = items[i]
		var tw = create_tween()
		tw.tween_property(it, "position", _anchor_pos(i, items.size()), 0.3)\
...
func _on_item_dropped(item: Node2D, drop_pos: Vector2) -> void:
	...
	if drop_pos.y < screen_h * (1.0 - drop_back_ratio):
		# Attempt placement
		var placed = get_tree().current_scene._spawn_hex(item.spawn_scene)
		if placed:
			# TODO: spend cost here, e.g. call your GameUI.spend(amount)
			items.erase(item)
			item.queue_free()
		else:
			# snap back for refund
			if not items.has(item):
				items.append(item)
	else:
		# cancelled drag – just return
		if not items.has(item):
			items.append(item)
	_reflow_items()
...
func _on_reroll_pressed() -> void:
	...
	for i in range(max_slots):
		...
		if i < items.size():
			old_item = items[i]
		var keep_frozen = old_item and old_item.frozen
		if keep_frozen:
